...
public class SagaOnlineStoreIntegrationTest
{
    private readonly AppFixture fixture;
    private DistributedApplication App => fixture.App;

    public SagaOnlineStoreIntegrationTest(AppFixture fixture)
    {
    ...
    public async Task Create_OnlineStore_Order(decimal productPrice, int quantityInStock, int orderItemQuantity, decimal bankAccountBalance, OrderStatus expectedOrderStatus)
    {
        ...
        var response = await catalogHttpClient.PostAsJsonAsync("/api/saga/v1/products", product, cancellationToken: TestContext.Current.CancellationToken);
        ...
        response = await inventoryHttpClient.PutAsJsonAsync($"/api/saga/v1/inventory/items/{product.Id}/restock", restockItem, cancellationToken: TestContext.Current.CancellationToken);

        // Assert
        Assert.Equal(HttpStatusCode.OK, response.StatusCode);

        // Act
        var inventoryItemResponse = await inventoryHttpClient.GetAsync($"/api/saga/v1/inventory/items/{product.Id}", TestContext.Current.CancellationToken);
        var inventoryItem = await inventoryItemResponse.Content.ReadFromJsonAsync<InventoryItem>(cancellationToken: TestContext.Current.CancellationToken);
        ...
        response = await paymentHttpClient.PostAsJsonAsync("/api/saga/v1/cards", card, cancellationToken: TestContext.Current.CancellationToken);

        // Assert
        card = await response.Content.ReadFromJsonAsync<Card>(cancellationToken: TestContext.Current.CancellationToken);
        Assert.NotNull(card);
        ...
        // Act
        response = await paymentHttpClient.GetAsync($"/api/saga/v1/cards/{cardId}", TestContext.Current.CancellationToken);
        card = await response.Content.ReadFromJsonAsync<Card>(cancellationToken: TestContext.Current.CancellationToken);
        ...
        await orderHttpClient.PostAsJsonAsync("/api/saga/v1/orders", order, cancellationToken: TestContext.Current.CancellationToken);

        await Task.Delay(2000, TestContext.Current.CancellationToken);

        var orderResponse = await orderHttpClient.GetAsync($"/api/saga/v1/orders/{order.Id}", TestContext.Current.CancellationToken);
        var orderResult = await orderResponse.Content.ReadFromJsonAsync<Order>(cancellationToken: TestContext.Current.CancellationToken);
        ...
        if (expectedOrderStatus == OrderStatus.Created)
        {
            ...
            inventoryItemResponse = await inventoryHttpClient.GetAsync($"/api/saga/v1/inventory/items/{product.Id}", TestContext.Current.CancellationToken);
            inventoryItem = await inventoryItemResponse.Content.ReadFromJsonAsync<InventoryItem>(cancellationToken: TestContext.Current.CancellationToken);
            ...
            var cardResponse = await paymentHttpClient.GetAsync($"/api/saga/v1/cards/{card.Id}", TestContext.Current.CancellationToken);
            var cardResult = await cardResponse.Content.ReadFromJsonAsync<Card>(cancellationToken: TestContext.Current.CancellationToken);
